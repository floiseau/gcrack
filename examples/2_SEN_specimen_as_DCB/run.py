...
class GCrackData(GCrackBaseData):
    ...
    def define_imposed_displacements(
        ...
        # Define function to locate the locked point (x = L, y = 0)
        def on_locked_point(x):
            return np.logical_and(np.isclose(x[0], self.pars["L"]), np.isclose(x[1], 0))
        ...
        # Define function to locate the left bot boundary (x=0, 0.5 < y <= L)
        def on_lbot_boundary(x):
            return np.logical_and(np.isclose(x[0], 0), x[1] < pars["L"] / 2)

        # Define imposed displacement boundary condition for the y-component
        comp = 1
        lbot_dofs = fem.locate_dofs_geometrical(
            (V_u.sub(comp), V_u.sub(comp).collapse()[0]), on_lbot_boundary
        )
        lbot_func = fem.Function(V_u.sub(comp).collapse()[0])
        with lbot_func.vector.localForm() as bc_local:
            bc_local.set(-1.0)
        lbot_bc = fem.dirichletbc(lbot_func, lbot_dofs, V_u)

        # Define function to locate the left top boundary (x=0, 0.5 < y <= L)
        def on_ltop_boundary(x):
            return np.logical_and(np.isclose(x[0], 0), x[1] > pars["L"] / 2)

        # Define imposed displacement boundary condition for the y-component
        comp = 1
        ltop_dofs = fem.locate_dofs_geometrical(
            (V_u.sub(comp), V_u.sub(comp).collapse()[0]), on_ltop_boundary
        )
        ltop_func = fem.Function(V_u.sub(comp).collapse()[0])
        with ltop_func.vector.localForm() as bc_local:
            bc_local.set(1.0)
        ltop_bc = fem.dirichletbc(ltop_func, ltop_dofs, V_u)

        # Return the list of Dirichlet boundary conditions
        return [locked_bc, lbot_bc, ltop_bc]
